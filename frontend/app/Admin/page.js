    ...
    <>
      ...
      <div className="min-h-screen bg-[#0B1220] px-6 py-10">
        <div className="max-w-7xl mx-auto">
          {/* Header Banner */}
          <section className="bg-gradient-to-r from-emerald-900 via-[#13253B] to-[#161F33] rounded-[32px] p-8 md:p-10 mb-10 border border-white/10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={loading}
                className="flex items-center gap-2 bg-[#24314C] hover:bg-[#2e3e5e] transition text-white px-5 py-3 rounded-2xl font-semibold text-sm border border-white/10"
              >
                <RefreshCw size={16} className={loading ? "animate-spin" : ""} /> Refresh
              </button>
              <button
                onClick={handleLogout}
                className="flex items-center gap-2 bg-rose-600 hover:bg-rose-700 transition text-white px-5 py-3 rounded-2xl font-semibold text-sm shadow-lg shadow-rose-900/30"
              >
          ...
          {/* Navigation Tabs */}
          <div className="flex items-center gap-3 mb-8 overflow-x-auto pb-2 border-b border-white/10">
            <button
            ...
            >
              <TrendingDown size={18} /> Fulfillment Failure Analytics
            </button>
          </div>
          ...
          {activeTab === "overview" && (
            <>
              ...
              {fetchFailed && (
                <div className="mb-8 rounded-2xl bg-rose-500/10 border border-rose-500/30 p-4 text-rose-300 text-sm font-medium">
              ...
              {!loading && (
                <div className="mb-12">
                  <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
                    <Building2 className="text-emerald-400" /> Branch Inventory Status
                  </h2>
          ...
          {activeTab === "lowStock" && (
            <div className="bg-[#161F33] rounded-[28px] border border-white/10 p-8 shadow-xl">
              <div className="flex justify-between items-center mb-8">
                <div>
                  <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                    <AlertTriangle className="text-amber-400" /> Low Stock Inventory Audit Report
                  </h2>
                ...
                {lowStockData && (
                  <span className="bg-amber-500/20 text-amber-300 border border-amber-500/30 px-4 py-1.5 rounded-full text-xs font-bold font-mono">
              ...
              {!lowStockData ? (
                ...
                <div className="space-y-8">
                  {(lowStockData.branches || []).map((b) => (
                    <div key={b.branch_id} className="bg-[#0D1527] p-6 rounded-2xl border border-white/5">
                      <div className="flex items-center justify-between mb-4 border-b border-white/10 pb-3">
                        <div>
                          <h3 className="text-lg font-bold text-white">{b.branch_name}</h3>
                        ...
                        <table className="w-full text-left text-sm text-gray-300">
                          <thead>
                            <tr className="text-xs text-gray-400 uppercase border-b border-white/10">
                              ...
                              <th className="pb-2">Rx Required?</th>
                              <th className="pb-2">Available Quantity</th>
                          ...
                          <tbody>
                            {(b.low_stock_items || []).map((item) => (
                              <tr key={item.medicine_id} className="border-b border-white/5">
                                <td className="py-2.5 font-semibold text-white">{item.medicine_name}</td>
          ...
          {activeTab === "failures" && (
            <div className="bg-[#161F33] rounded-[28px] border border-white/10 p-8 shadow-xl">
              <div className="mb-8">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                  <TrendingDown className="text-rose-400" /> Branch Fulfillment Failure Analytics
                </h2>
                <p className="text-gray-400 text-sm mt-1">
                      ...
                      {(failureData.failing_branches_summary || []).map((fb) => (
                        <tr key={fb.branch_id} className="border-b border-white/5 hover:bg-[#1f2d47]/50 transition">
                          <td className="py-4 px-3 font-bold text-white">
                            {fb.branch_name}
                            <span className="block text-xs text-gray-400 font-normal">{fb.branch_location}</span>
                          </td>
                          <td className="py-4 px-3 font-mono font-bold text-white">{fb.total_orders}</td>
                          <td className="py-4 px-3 font-mono text-rose-400 font-bold">{fb.failed_orders}</td>
                          <td className="py-4 px-3">
                            <span className="bg-rose-500/20 text-rose-300 border border-rose-500/30 px-3 py-1 rounded-full text-xs font-bold font-mono">
                              {fb.failure_rate_percentage}%
                            </span>
                          </td>
                          <td className="py-4 px-3 font-mono text-rose-400">{fb.out_of_stock_count}</td>
                          <td className="py-4 px-3 font-mono text-amber-400">{fb.low_stock_count}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {selectedBranchToday && (
        <div className="fixed inset-0 z-[999] bg-black/80 flex items-center justify-center p-4">
          <div className="bg-[#161F33] rounded-[30px] border border-white/20 p-8 max-w-3xl w-full max-h-[85vh] overflow-y-auto relative shadow-2xl">
            <button
              onClick={() => {
            ...
            <h2 className="text-2xl font-bold text-white flex items-center gap-2 mb-2">
              <Calendar className="text-emerald-400" /> Today&apos;s Orders — {selectedBranchToday.name}
            </h2>
            ...
            {todayLoading ? (
              ...
              <div className="p-8 text-center text-gray-400 bg-[#0D1527] rounded-2xl">
                No orders received today for this branch yet.
              </div>
            ) : (
              <div className="space-y-4">
                {(todayOrdersModal.orders || []).map((o) => (
                  <div key={o.order_id} className="bg-[#0D1527] p-5 rounded-2xl border border-white/5">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <span className="text-xs font-mono text-emerald-400">Order #{o.order_id}</span>
                        <h4 className="text-lg font-bold text-white">{o.customer_name}</h4>
